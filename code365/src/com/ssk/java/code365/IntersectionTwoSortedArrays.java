package com.ssk.java.code365;

import java.util.ArrayList;
import java.util.Arrays;
import java.util.List;

/*

Problem Description
Find the intersection of two sorted arrays OR in other words, given 2 sorted arrays, find all the elements which occur in both arrays.
NOTE: For the purpose of this problem ( as also conveyed by the sample case ), assume that elements that appear more than once in both arrays should be included multiple times in the final output.

Problem Constraints
	1 <= |A| <= 106
	1 <= |B| <= 106

Input Format
The first argument is an integer array A.
The second argument is an integer array B.

Output Format
Return an array of intersection of the two arrays.

Example Input
	Input 1:
	A: [1 2 3 3 4 5 6]
	B: [3 3 5]

	Input 2:
	A: [1 2 3 3 4 5 6]
	B: [3 5]

Example Output
	Output 1: [3 3 5]
	Output 2: [3 5]

Example Explanation
	Explanation 1:
	3, 3, 5 occurs in both the arrays A and B
	Explanation 2:
	Only 3 and 5 occurs in both the arrays A and B


Algorithm : using 2 pointer method
    Create 2 pointers i,j which are pointeing to 0th position of each list inputs.
    check for < , > and = cases and adjust corresponding pointers on both lists
    At = case: upate the final out with any of the list value to capture intersection.

*/
public class IntersectionTwoSortedArrays {
    public static void main(String[] args) {
        List<Integer> A = Arrays.asList(1, 2, 3, 3, 4, 5, 6);
        List<Integer> B = Arrays.asList(3, 3, 5);
        System.out.println("Input lists : ");
        System.out.println(A);
        System.out.println(B);
        List<Integer> output = new ArrayList<>();
        int i=0,j=0;
        while(i<A.size() &&j<B.size()){
            if(A.get(i)<B.get(j)){
                i++;
            }else if(B.get(j)<A.get(i)){
                j++;
            }else {  // Intersection case at both list so any one value.
                output.add(A.get(i));
                i++;
                j++;
            }
        }
        System.out.print("Inter Section of above two arrays : ");
        output.forEach(x-> System.out.print(x+" "));
    }
}
